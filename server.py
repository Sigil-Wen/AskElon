        ...
        if data['event'] == "media":
            if not has_seen_media:
                ...
                app.logger.info("Additional media messages from WebSocket are being suppressed....")
                 ...
                 # Generate a response using Twilio's VoiceResponse and Say classes
                resp = VoiceResponse()
                resp.say("Hello, World!")

                # Send the response back to the caller via the websocket
                response_data = {
                    "event": "media",
                    "media": {
                        "payload": base64.b64encode(resp.to_xml().encode('utf-8')).decode('ascii')
                    }
                }
